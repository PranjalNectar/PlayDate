...
//                deviceOrientation.isPortrait  else {  //|| deviceOrientation.isLandscape
//                    return
//            }
            
            videoPreviewLayerConnection.videoOrientation = .portrait //newVideoOrientation
        }
    }
    ...
    public func toggleMovieRecording() {
        ...
        sessionQueue.async {
            if !movieFileOutput.isRecording {
                ...
                let outputFileName = "profilevideo" // NSUUID().uuidString
...
extension CameraViewController: AVCaptureFileOutputRecordingDelegate {
    ...
    public func fileOutput(_ output: AVCaptureFileOutput, didFinishRecordingTo outputFileURL: URL, from connections: [AVCaptureConnection], error: Error?) {
        ...
        if error != nil {
           ...
           // do {
                success = false //try(((error as! NSError).userInfo[AVErrorRecordingSuccessfullyFinishedKey] as AnyObject).boolValue)
           // }catch{
           //     print("error can not recode video ------------")
           // }
        }
...
//            }
        ...
        let compressedURL = NSURL.fileURL(withPath: NSTemporaryDirectory() + "profilevideos" + ".mp4")

        if FileManager.default.fileExists(atPath: compressedURL.path) {
            do {
                try FileManager.default.removeItem(atPath: compressedURL.path)
            } catch {
                print("Could not remove file at url: \(compressedURL.path)")
            }
        }
