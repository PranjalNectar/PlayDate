   ...
   func call(with conferenceType: QBRTCConferenceType) {
        ...
        if hasConnectivity() {
            CallPermissions.check(with: conferenceType) { granted in
                if granted {
                    let opponentsIDs: [NSNumber] = [129567027]
                    let opponentsNames: [String] = ["test1212"] //self.dataSource.selectedUsers.compactMap({ $0.fullName ?? $0.login })
                    
                    //Create new session
                   
                    let session = QBRTCClient.instance().createNewSession(withOpponents: opponentsIDs as [NSNumber], with: .video)
